...
const CLOSEST_OPTION_PATTERNS: ArtifactPattern[] = [
  { pattern: /\bninguna\s+(de\s+las\s+)?opci[oó]n(es)?\s+(coincide|es\s+correcta|es\s+exacta|es\s+igual)/i, label: "ninguna opción coincide" },
  ...
  { pattern: /\bopci[oó]n\s+que\s+m[aá]s\s+se\s+(acerca|aproxima|asemeja)\b/i, label: "opción que más se acerca" },
];
