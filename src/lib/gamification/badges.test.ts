...
function ctx(overrides: Partial<BadgeContext>): BadgeContext {
  return {
    overall: {
      ...
      activeTopics: 0, topicsTouched: 0,
    },
    aggregates: [],
...
describe("computeBadges", () => {
  ...
  it("primer ejercicio → first-steps ganado", () => {
    const badges = computeBadges(ctx({
      overall: { totalAttempts: 1, correctAttempts: 1, accuracy: 100, activeTopics: 1, topicsTouched: 1 },
    }));
  ...
  it("sharp solo cuenta con ≥20 intentos", () => {
    ...
    const a = computeBadges(ctx({
      overall: { totalAttempts: 10, correctAttempts: 9, accuracy: 95, activeTopics: 1, topicsTouched: 1 },
    }));
    expect(a.find((b) => b.def.id === "sharp")!.earned).toBe(false);

    // 95% con 25 attempts → califica
    const b = computeBadges(ctx({
      overall: { totalAttempts: 25, correctAttempts: 24, accuracy: 95, activeTopics: 2, topicsTouched: 2 },
    }));
  ...
  it("progress nunca excede 100", () => {
    const badges = computeBadges(ctx({
      overall: { totalAttempts: 1000, correctAttempts: 1000, accuracy: 100, activeTopics: 5, topicsTouched: 11 },
...
describe("badgeStats", () => {
  it("cuenta ganados y total", () => {
    const badges = computeBadges(ctx({
      overall: { totalAttempts: 1, correctAttempts: 1, accuracy: 100, activeTopics: 0, topicsTouched: 1 },
    }));
