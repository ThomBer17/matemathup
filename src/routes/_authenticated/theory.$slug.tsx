...
import {
  ArrowLeft, Play, ListChecks, AlertTriangle, BookText, Lightbulb,
  Sigma, CheckCircle2, Library, Target,
} from "lucide-react";
...
function TheoryDetail() {
  ...
  return (
    <article className="mx-auto max-w-3xl px-6 py-8 md:py-12">
      <Link to="/theory" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
        <ArrowLeft className="h-3.5 w-3.5" /> Teoría
      </Link>

      {/* Encabezado */}
      <header className="mt-4 flex flex-wrap items-start gap-4">
        <div className={`grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-gradient-to-br ${topicGradient(topic?.color)}`}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <h1 className="mt-1 font-display text-3xl font-bold tracking-tight md:text-4xl">{name}</h1>
          {topic?.description && (
      ...
      {/* 1. Resumen */}
      <Section icon={BookText} title="Resumen">
        <div className="space-y-3 text-[15px] leading-[1.75] text-foreground/90">
          {theory.summary.split("\n\n").map((p, i) => (
            <p key={i}><MathRich text={p} /></p>
          ))}
        </div>
      </Section>

      {/* 2. Conceptos clave */}
      <Section icon={Lightbulb} title="Conceptos clave">
        <div className="flex flex-wrap gap-2">
          {theory.keyConcepts.map((c) => (
            <span key={c} className="rounded-lg border bg-card px-3 py-1.5 text-sm font-medium shadow-soft">
              <MathRich text={c} />
            </span>
          ))}
        </div>
      ...
      {/* 3. Fórmulas */}
      <Section icon={Sigma} title="Fórmulas importantes">
      ...
      <Section icon={BookText} title="Ejemplos resueltos">
      ...
      {/* 6. Checklist */}
      <Section icon={ListChecks} title="Checklist de repaso">
      ...
      {topic && (
        <div className="mt-12 flex flex-col items-center gap-3 rounded-2xl border bg-gradient-to-br from-primary-soft/60 to-card p-6 text-center shadow-soft">
          ...
          <p className="max-w-md text-sm text-muted-foreground">
            La mejor forma de fijar la teoría es resolviendo. Probá ejercicios adaptados a tu nivel.
          </p>
          <Link
            to="/topics/$slug"
            params={{ slug }}
...
function Section({
  icon: Icon, title, children, tone = "default",
}: {
  ...
  return (
    ...
    >
      <h2 className="flex items-center gap-2 font-display text-xl font-bold tracking-tight">
        <Icon className={cn("h-5 w-5", tone === "warning" ? "text-warning-foreground/80" : "text-primary")} />
        {title}
      </h2>
