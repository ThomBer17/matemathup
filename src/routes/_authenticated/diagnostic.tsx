  ...
  const finish = async (all: AnswerRecord[]) => {
    ...
    if (user) {
      try {
        ...
        const { error } = await supabase.rpc("submit_diagnostic_results", {
          p_results: computed.map((r) => ({
...
function DiagnosticResults({
  ...
  topicBySlug: Map<
    string,
    { id: string; slug: string; name: string; color: string | null; icon: string | null }
  >;
  saving: boolean;
}) {
  const { correct, total } = summarize(
    results.map((r) => ({
      outcome: (r.masteryPct >= 45 ? "correct" : "incorrect") as DiagnosticOutcome,
    })),
  );
    ...
    <div className="mx-auto max-w-2xl px-6 py-8 md:py-12">
      ...
      >
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Acertaste <span className="font-semibold tabular-nums text-foreground">{correct}</span> de{" "}
          {total}. Calibramos tu punto de partida en cada tema.
        </p>
      ...
      <div className="mt-8 space-y-2">
        ...
        {sorted.map((r) => {
          ...
          return (
            ...
            >
              <div
                className={`grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-gradient-to-br ${topicGradient(topic.color)}`}
              >
