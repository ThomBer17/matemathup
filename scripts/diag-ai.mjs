...
const apiKey = process.argv[2] || (process.env.AI_API_KEY && !process.env.AI_API_KEY.includes("PEGÁ") ? process.env.AI_API_KEY : null);
...
function fixJsonBackslashes(raw) {
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    if (ch === "\\") {
      ...
      if (next === "n" && !/[a-z]/.test(raw[i + 2] ?? "")) { out += "\\n"; i++; continue; }
      out += "\\\\"; continue;
    }
    if (ch === "\n") { out += "\\n"; continue; }
...
const configs = [
  { name: "A: reasoning_effort=none + max_tokens=8192", extra: { reasoning_effort: "none", max_tokens: 8192 } },
...
for (const cfg of configs) {
  ...
  for (const [n, c] of Object.entries(stages)) {
    try { JSON.parse(c); console.log(`  parse(${n}): OK`); }
    catch (e) { console.log(`  parse(${n}): FALLA → ${e.message}`); }
  }
